...
Deno.serve(async (req: Request) => {
    ...
    // VAPID: o secret deve ser o JSON exportado { publicKey, privateKey }
    let vapidKeys: webpush.VapidKeys;
    try {
        const parsed = JSON.parse(vapidPrivate);
        vapidKeys = await webpush.importVapidKeys(parsed, { extractable: false });
    } catch (e) {
